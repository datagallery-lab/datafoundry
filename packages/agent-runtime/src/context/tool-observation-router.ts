...
import type { ToolResultDispatcher } from "./tool-result-dispatcher.js";

export type ToolObservationRouterOptions = {
  dispatcher: ToolResultDispatcher;
...
export class ToolObservationRouter {
  private readonly governedResults = new Map<string, unknown>();
  private readonly missingAdapters = new Set<string>();

  constructor(private readonly options: ToolObservationRouterOptions) {}

  /** Route completed tool observations through their registered adapters. */
  ...
  private governPart(part: unknown): unknown {
    ...
    try {
      ...
      this.governedResults.set(observation.key, contextPackage.model);
      return observation.replaceResult(contextPackage.model);
    } catch (error) {
  ...
  private emitMissingAdapter(toolName: string): void {
    ...
    this.options.emitter.emit(createCustomEvent("context.adapter-missing", { tool_name: toolName }));
  }
}
