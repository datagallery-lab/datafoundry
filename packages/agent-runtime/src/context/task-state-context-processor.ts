...
export type TaskStateContextProcessorOptions = {
  runtime: TaskStateRuntime;
  threadId: string;
};

export class TaskStateContextProcessor implements Processor<"task-state-context"> {
  readonly id = "task-state-context";
  readonly name = "Task State Context Processor";

  constructor(private readonly options: TaskStateContextProcessorOptions) {}
