...
function isLikelyPaste(input: string, key: Key): boolean {
  if (!input || key.return || key.backspace || key.delete || key.escape || isNavigationKey(key)) {
    return false;
  }
...
export const EnhancedInputBox: React.FC<EnhancedInputBoxProps> = ({
  ...
  const renderInputLine = (lineText: string, index: number) => {
    ...
    const isCursorLine = index === relativeCursorRow;
    ...
    const codePoints = toCodePoints(displayText);
