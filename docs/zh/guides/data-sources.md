...
配置的 PostgreSQL `schema` 是元数据抓取和表预览唯一使用的 schema，同时会作为 Agent 只读 SQL
事务内的 `search_path`。表注释和列注释通过 `description` 字段返回。可以通过下列接口预览表数据首页：

```bash
curl "http://127.0.0.1:8787/api/v1/datasources/sales-pg/tables/orders/preview?limit=50&offset=0"
```

保存新的连接配置后，旧 schema 快照会失效，数据源状态会回到“未测试”。在 Agent run 中使用前应重新测试连接。

对 PostgreSQL，`maxRows` 会在服务端 cursor 拉取数据时强制执行，即使提交的 SQL 带有更大的 `LIMIT`。
`maskFields` 会根据 PostgreSQL 源列识别直接别名；启用脱敏策略时，无法确认来源的派生列会被保守遮罩。
配置 `tableAllowlist` 后，系统会拒绝无法可靠确认 `FROM` 关系的查询。数据库账号权限仍是首要安全边界。
allowlist 无法检查数据库函数内部动态执行的 SQL，因此应使用最小权限账号，并撤销可执行任意 SQL 的函数
`EXECUTE` 权限。
...
- 将表 allowlist 作为纵深防护；敏感表仍需通过数据库授权和函数执行权限进行保护。
