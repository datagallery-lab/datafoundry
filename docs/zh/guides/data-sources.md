...
Open Data Agent 通过 Data Gateway 管理数据源。数据源可以在 Web、TUI 或 REST API 中注册和测试，但真正的数据分析仍由 Agent 通过受控工具执行。
...
| `trino` / `presto` | Trino 或 Presto REST statement。 |
| `spark` | Spark Thrift Server / HiveServer2。 |
| `databricks` | Databricks SQL Statement REST。 |
| `elasticsearch` / `opensearch` | index-as-table 查询边界。 |
| 兼容型数据库 | GaussDB、StarRocks、Redshift、Doris、MariaDB、TiDB、OceanBase、Greenplum 等复用 PostgreSQL 或 MySQL 协议适配。 |

部分外部服务适配器已经接通配置契约，但真实环境验证需要对应服务、网络和凭据。对外演示建议优先使用 DuckDB demo、SQLite、CSV / Excel、PostgreSQL 或 MySQL。
