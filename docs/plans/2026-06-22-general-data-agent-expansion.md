...
4. Mastra Workspace 自动注入的工具结果在 step-level 统一拦截，再按 `toolName` 路由到一工具一个
   `ToolObservationAdapter`；通用层只负责路由和未知工具 fallback。
...
- 每个工具通过自己的 `ToolObservationAdapter` 进入每一步 prompt 编译和预算治理。
...
4. `Agent` 配置 `workspace` 后，Mastra 会注入 Workspace tools，但这些工具不会自动调用我们的
   `ToolObservationPackager.packageToolObservation()`；需要从 Mastra tool observation 统一提取并路由到已注册 adapter。
...
### 5.3 MastraToolObservationRouter 与一工具一 Adapter

“一工具一个 `ToolObservationAdapter`”是既定扩展边界，继续保留。统一层只解决 Mastra 自动工具不会主动调用
`ToolObservationPackager.packageToolObservation()` 的接入问题，不能取代各工具 adapter。

```text
Mastra messages
  -> MastraToolObservationRouter.governMessages()
  -> ToolObservationDispatcher.dispatch(toolName, rawResult)
  -> ToolObservationPackager.packageToolObservation()
  -> ToolObservationAdapterRegistry.resolveByToolName(toolName)
  -> concrete ToolObservationAdapter.toContextItems(rawResult)
...
- `MastraToolObservationRouter`：稳定解析 Mastra tool message、去重 tool call、查找 adapter、记录缺失 adapter。
- `ToolObservationDispatcher`：把工具执行边界和 next-step message 路由统一到同一治理入口，并标记已治理对象避免二次处理。
- `ToolObservationPackager`：分配预算、调用 adapter、执行 policy/redaction，并注册 observation 到 `ContextRunState`。
- `ToolObservationAdapterRegistry`：按精确 `toolName` 注册和解析 adapter，重复注册直接失败。
- `ToolObservationAdapter`：理解该工具的 input/output schema，并生成 model/activity/audit 三种投影所需的 `ContextItem`。
...
| Adapter | Tool | 专属治理 |
|---|---|---|
| `InspectSchemaToolObservationAdapter` | `inspect_schema` | schema 结构裁剪、表列优先级 |
| `RunSqlReadonlyToolObservationAdapter` | `run_sql_readonly` | rows 预览、audit/artifact ref |
| `ReadFileToolObservationAdapter` | `read_file` | mime、文本截断、大文件引用化 |
| `WriteFileToolObservationAdapter` | `write_file` | 路径、大小、hash，不回灌文件全文 |
| `EditFileToolObservationAdapter` | `edit_file` | 修改摘要、路径、版本信息 |
| `ListFilesToolObservationAdapter` | `list_files` | 数量、深度和 metadata 限制 |
| `GrepToolObservationAdapter` | `grep` | 命中数、路径、行号和片段预算 |
| `SearchToolObservationAdapter` | `search` | score、来源和 top-k 限制 |
| `ExecuteCommandToolObservationAdapter` | `execute_command` | stdout/stderr/exitCode 分离治理 |
| `RegisterOutputToolObservationAdapter` | `register_output` | artifact manifest ref |
| `ExportQueryResultToolObservationAdapter` | `export_query_result` | 导出摘要、audit/artifact ref |
| `TaskWriteToolObservationAdapter` | `task_write` | 稳定 task ID 和完整替换结果 |
| `TaskUpdateToolObservationAdapter` | `task_update` | 单 task 变更和当前快照 |
| `TaskCompleteToolObservationAdapter` | `task_complete` | 完成状态和剩余任务 |
| `TaskCheckToolObservationAdapter` | `task_check` | completion summary |
| `AskUserToolObservationAdapter` | `ask_user` | 问题、选项、resume answer |
| `SubmitPlanToolObservationAdapter` | `submit_plan` | plan、审批结果、feedback |
...
- 实现 `MastraToolObservationRouter`，并为每个首期 Workspace 工具注册独立 `ToolObservationAdapter`。
...
- [x] 实现四个独立 task `ToolObservationAdapter`。
...
export 中不存在该符号。现阶段使用 `MastraTaskStateContextProcessor` 从同一原生 threadState store 读取并在每个 step
...
- [x] 接入两个 builtin tool 及各自 `ToolObservationAdapter`。
...
- **决定**：在 `processInputStep()` 提取 observation，再按 toolName 路由给独立 `ToolObservationAdapter`。
