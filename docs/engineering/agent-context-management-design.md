...
- `PromptView` 是发给当前模型调用的临时投影，不作为长期记忆或权威上下文。
...
- ContextPackage、ContextPlan 和 PromptView 的审计与可观测性。
...
-> ContextBudgetProcessor.processInputStep
-> groupMessagesByTurn
-> StepContextPlanner
-> current-step PromptView
...
- [agent-context-current.puml](./agent-context-current.puml)
- [agent-context-current.svg](./agent-context-current.svg)
...
所有来源最终转换为统一 `ContextItem`；conversation 直接从 Mastra MessageList 归一化，外部数据来源通过
adapter 进入：

| 来源 | Adapter | 主要输出 |
| --- | --- | --- |
| Mastra messages | `groupMessagesByTurn` + `ContextBudgetProcessor` | turn、assistant/tool exchange、当前用户消息 |
| Agent instructions | `SystemInstructionAdapter` | runtime policy、datasource context |
| Mastra tools | `ToolSchemaAdapter` | 工具名称、描述、input schema |
| Tool execution | `ToolObservationAdapter` | 有界 observation、artifact/audit ref |
| Mastra memory | `MemorySourceAdapter` | recent memory、working memory、summary |
| Knowledge | `KnowledgeSourceAdapter` | retrieval chunk、citation、artifact ref |
...
`ContextBudgetProcessor` 实现 Mastra `Processor.processInputStep()`，每个 ReAct step 接收：
...
`ProviderPromptGuardProcessor.processLLMRequest()` 位于 provider 调用前，处理已经转换成
...
- 保留 adapter 的确定性来源治理。
- 增加 `ContextPlan`、`PromptView` 和 `ContextDecision` contract。

### Phase 2：Mastra Step Processor

状态：已实现。

- 实现 `ContextBudgetProcessor.processInputStep()`。
...
- 增加 `ProviderPromptGuardProcessor`。
...
- [Phase 4-5 扩展组件图](./agent-context-target-component.svg)
- [已实现类图](./agent-context-target-class.svg)
- [当前运行时序图](./agent-context-target-sequence.svg)
- [编译状态图](./agent-context-target-state.svg)
