...
| Tool | Adapter | 主要投影 |
| --- | --- | --- |
| `list_data_sources` | `ListDataSourcesToolObservationAdapter` | run allowlist 数据源摘要 |
| `inspect_schema` | `SchemaToolObservationAdapter` | schema 裁剪和 run-local `schema_id` |
| `preview_table` | `PreviewTableToolObservationAdapter` | bounded preview |
| `run_sql_readonly` | `SqlResultToolObservationAdapter` | 行列裁剪、artifact/audit ref |
| `read_file` | `ReadFileToolObservationAdapter` | 文件内容限额 |
| `write_file` | `WriteFileToolObservationAdapter` | 写入摘要 |
| `edit_file` | `EditFileToolObservationAdapter` | 编辑摘要 |
| `list_files` | `ListFilesToolObservationAdapter` | 文件列表裁剪 |
| `grep` | `GrepToolObservationAdapter` | 命中结果裁剪 |
| `file_stat` | `FileStatToolObservationAdapter` | 元数据投影 |
| `mkdir` | `MkdirToolObservationAdapter` | 创建结果摘要 |
| `execute_command` | `ExecuteCommandToolObservationAdapter` | stdout/stderr/exit code 限额 |
| `task_write` | `TaskWriteToolObservationAdapter` | 完整 task snapshot |
| `task_update` | `TaskUpdateToolObservationAdapter` | task 变化和 snapshot |
| `task_complete` | `TaskCompleteToolObservationAdapter` | 完成状态和 snapshot |
| `task_check` | `TaskCheckToolObservationAdapter` | summary 和 incomplete tasks |
| `ask_user` | `AskUserToolObservationAdapter` | resume answer 和错误摘要 |
| `submit_plan` | `SubmitPlanToolObservationAdapter` | approval/rejection 和 feedback 摘要 |

## 新工具统一模板

1. 在工具目录实现副作用和 raw result schema，不调用 context adapter。
2. 新建唯一 `ToolObservationAdapter`，只负责该工具结果的裁剪、脱敏和分层投影。
3. 在 `ToolObservationAdapterRegistry` 注册 adapter；重复注册直接失败。
