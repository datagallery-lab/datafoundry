...
少量仍有维护价值的工程契约放在仓库 `.docs-internal/` 目录。这些文件可随开源仓提交，但不是公开阅读入口，也不能包含凭据、个人路径、个人信息、客户信息或来源敏感描述。

## 维护规则

- 公开文档不要链接内部文档区。
- 新增对外能力时，优先更新 `zh/` 下的中文文档。
- 示例凭据只能使用示例值，例如 `replace-with-your-key`、`<dev_token>`。
- 不保留来源敏感叙述。
