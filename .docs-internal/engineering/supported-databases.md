...
这些类型不是 SQL 引擎。它们接入 `run_sql_readonly` 的含义是“通过 SQL-like 语法表达只读投影”，
不是开放原生命令入口。Data Gateway 只接受下表中的简单 `SELECT` 形态，然后由 adapter 转成只读
SDK 调用；Mongo aggregation、Redis 写命令、Elasticsearch/OpenSearch DSL 都不会暴露给 agent。
